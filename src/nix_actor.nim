...
proc newStoreEntity(turn: Turn; detail: StoreResolveDetail): StoreEntity =
  ...
  entity.self.relay.onStopdo (turn: Turn):
    entity.store.close()
  entity
...
proc newEvalEntity(turn: Turn; detail: EvalResolveDetail): EvalEntity =
  ...
  let entity = EvalEntity(store: openStore(detail.storeUri.get("auto"),
      detail.storeParams))
...
proc newChild(parent: EvalEntity; turn: Turn; val: NixValue): EvalEntity =
  ...
  let entity = EvalEntity(store: parent.store, state: parent.state, root: val)
...
proc bootActor*(turn: Turn; relay: Cap) =
  ...
  gk.servedo (turn: Turn; step: StoreResolveStep) -> rpc.Result:
    newStoreEntity(turn, step.detail).self.resultOk
  gk.servedo (turn: Turn; step: EvalResolveStep) -> rpc.Result:
