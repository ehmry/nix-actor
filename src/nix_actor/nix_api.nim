...
import
  ./nix_api_expr, ./nix_api_store, ./nix_api_types, ./nix_api_value, ./utils
...
proc close*(value: Value) =
  mitNix:(discard nix.gc_decref(cast[pointer](value)))

proc evalFromString*(nix: NixContext; state: EvalState; expr, path: string;
                     result: Value) =
...
proc force*(state: EvalState; value: Value) =
  mitNix:(discard nix.value_force(state, value))
