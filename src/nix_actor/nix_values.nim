...
import
  std / options, pkg / preserves, ./[nix_api, nix_api_util, nix_api_value]
...
proc toPreserves*(value: NixValue; state: EvalState): Value {.gcsafe.} =
  ...
  of NIX_TYPE_ATTRS:
    if has_attr_byname(ctx, value, state, "drvPath"):
      result = initRecord("drv", get_attr_byname(ctx, value, state, "drvPath").toPreserves(
          state), get_attr_byname(ctx, value, state, "outPath").toPreserves(
          state))
    else:
  ...
  try:
    ...
    of pkRecord:
      if pr.isRecord("null", 0):
        ...
        ctx.check bindings_builder_insert(ctx, b, "drvPath",
            translate(ctx, state, pr.fields[0]))
        ctx.check bindings_builder_insert(ctx, b, "outPath",
            translate(ctx, state, pr.fields[1]))
        ctx.check make_attrs(ctx, result, b)
      else:
        raise newException(ValueError,
                           "cannot convert Preserves record to Nix: " & $pr)
    of pkSequence, pkSet:
      let b = make_list_builder(ctx, state, pr.len.csize_t)
      defer:
        list_builder_free(b)
      for i, e in pr:
        ctx.check list_builder_insert(ctx, b, i.register.cuint,
                                      translate(ctx, state, e))
    ...
    of pkDictionary:
      ...
      for (name, value) in pr.dict:
        if name.isSymbol:
          ctx.check bindings_builder_insert(ctx, b, name.symbol.string,
              translate(ctx, state, value))
        else:
          ctx.check bindings_builder_insert(ctx, b, $name,
              translate(ctx, state, value))
      ctx.check make_attrs(ctx, result, b)
    of pkEmbedded:
      raise newException(ValueError,
